...
kafka_topics_FP = (
    "finapres_waveform2_s"  # XXX use finapres_waveform_s if you can't find any record
)
...
def init_meta():
    return nirscloud_util_meta.init(
        nirscloud_meta_logger,
        "meta",
        hostname=hostname,
        ssl=True,
        cert=pemkeyfile,
...
def query_fastrak_meta(query={}):
    return query_meta(
        ...
        fields=["meta_id", *fastrak_schema_fields],
    )
...
def query_nirs_meta(query={}):
    return query_meta(
        ...
        fields=["meta_id", *nirs_schema_fields],
    )
...
def convert_nano_ts_to_index_inplace(df: pd.DataFrame, timezone: Optional[str] = None):
    ...
    time_type = (
        f"datetime64[ns, {timezone}]" if timezone is not None else "datetime64[ns]"
    )
...
def read_nirs_ds_from_meta(meta_dict):
    ...
    meta_dur = (
        pd.to_timedelta(np.round(np.float64(meta_dict["duration"])), "s").to_numpy()
...
def load_nirs_ds(subject_id: str, the_date: str, with_calib: bool = True) -> xr.Dataset:
    ...
    nirs_dss = [
        read_nirs_ds_from_meta(meta).sortby("time")
