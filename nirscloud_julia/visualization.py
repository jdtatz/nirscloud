...
def interactive_bokeh_measurement_time_plot(
    measurement_ds,
    ft_locs=("head", "nirs"),
    ...
    # b/c there are duplicated measurment_ids, somehow???
    measurement_trials = [
        (k, [trials.isel(measurement=i) for i in range(len(trials.measurement_trial))])
        for k, trials in measurement_ds.groupby("measurement_location")
    ]
        ...
        (
            ...
            [
                [
                    {
                        "x": fastrak_time_s,
                        "y": trial_ds.position.sel(cartesian_axes=d, location=l),
                        "y_axis_label": f"{l}-{d} ({trial_ds.position.units})",
                        "title": f"Trial {trial_ds.measurement_trial.item()}",
                    }
                    for d, l in ft_idxs
                ]
                + [
                    {
                        "x": nirs_time_s,
                        "y": trial_ds.ac.sel(wavelength=w, rho=r),
                        "y_axis_label": f"ac [λ={w.item()}, ρ={r.item()}]",
                        "title": f"Trial {trial_ds.measurement_trial.item()}",
                    }
                    for w, r in nirs_idxs
                ]
...
class OrientationVisualization(art3d.Poly3DCollection):
    ...
    def __init__(
        ...
        initial_direction: np.ndarray = np.array((0, 0, 1)),
        **kwargs,
    ):
...
class FastrakVisualization:
    ...
    def interactive_3d_mean_plot(self, fig, cmap="Set2"):
        ...
        for i, (loc, trials) in enumerate(fastrak_mean_measurements_grouped.items()):
            ...
            for j in range(len(trials.measurement)):
                ...
                ax.scatter(
                    *trial.position.sel(location="nirs").T,
                    color=c,
                    depthshade=False,
                    s=s,
                    edgecolor="k",
        ...
        for i, (loc, trials) in enumerate(fastrak_mean_measurements_grouped.items()):
            ...
            for j in range(len(trials.measurement)):
                ...
                ax.scatter(
                    *trial.position.sel(location="relative").T,
                    color=c,
                    depthshade=False,
                    s=s,
                    edgecolor="k",
    ...
    def positioning_with_histogram_plot(
        self,
        fig,
        *,
        smooth=True,
        ...
        gs = fig.add_gridspec(1, 3 if plot_relative else 2)
        histo_kwargs = dict(
            time_slice=slice(None, t_max),
            smooth=smooth,
            hist_size=hist_size,
            nirs_cmap=nirs_cmap,
            nirs_alpha=nirs_alpha,
            tz=tz,
        )
        ...
        fig.suptitle(
            f"Subject {self.fastrak_ds.subject.item()} on {np.datetime_as_string(self.fastrak_ds.time[0], unit='D', timezone=tz if tz is not None else 'naive', casting='unsafe')}"
        )
        return lgd

    def interactive_measurement_time_plot(
        self,
        ft_locs=("head", "nirs", "relative"),
        wavelength_indices=(0,),
        rho_idxs=(0, -1),
        figsize=(16, 8),
    ):
        ...
        def interactive_update_wrapper(key):
            ...
            sub_gs, sub_axs = plot_measurment_trials(
                gs[0, 0],
                measurement_ds,
                ft_locs=ft_locs,
