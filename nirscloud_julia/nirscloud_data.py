...
def load_fastrak_fiducial_ds(mongo_client, webhdfs_client, note_id: str) -> xr.Dataset:
    ...
    for measurement_id_prefix, loc in zip(measurement_id_prefixs, fiducial_locs):
        *_, fastrak_meta = query_fastrak_meta(mongo_client, {'note_id.val': note_id, 'measurement_id.val': {"$regex": f"{measurement_id_prefix}_\\d+"},})
...
def load_fastrak_ds(mongo_client, webhdfs_client, subject_id: str, the_date: str) -> xr.Dataset:
    pencil_idx = 0
    head_idx=1
    nirs_idx=2
    measurement_id_prefix = "ftmsmt"
    *_, fastrak_meta = query_fastrak_meta(mongo_client, {'subject_id.val': subject_id, "the_date.val": the_date, 'measurement_id.val': {"$regex": f"{measurement_id_prefix}_\\d+"},})
    fastrak_ds = read_fastrak_ds_from_meta(webhdfs_client, fastrak_meta)
    ...
    fiducial_ds = load_fastrak_fiducial_ds(mongo_client, webhdfs_client, fastrak_ds.coords["note_id"].item())
    return fastrak_ds.assign_coords(fiducial_position=fiducial_ds.position, fiducial_orientation=fiducial_ds.orientation)


def load_nirs_ds(mongo_client, webhdfs_client, subject_id: str, the_date: str, with_calib: bool=True) -> xr.Dataset:
