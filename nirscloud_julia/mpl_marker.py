...
def centered_text_as_path(s: str) -> mpl.path.Path:
    ...
    # dy = font.get_size()
    w, h, _d = mpl.textpath.text_to_path.get_text_width_height_descent(
        s, fp, ismath=False
    )
    trans = (
        mpl.transforms.Affine2D.identity()
        .scale(fp.get_size() / mpl.textpath.text_to_path.FONT_SCALE)
        .translate(-w / 2, -h / 2)
    )
...
def marker_with_text(base_marker, text: str) -> markers.MarkerStyle:
    ...
    text_path = centered_text_as_path(text).transformed(
        mpl.transforms.Affine2D.identity().scale(3 / 4)
    )
